...
Deep mutation scan at Y537 of the holo ERα homodimer (1A52, both chains, both estradiol).
Y537 is mutated symmetrically on chain A and chain B. Estradiol occupies chains C and D.
...
Input:  inputs/1A52/1A52_clean_dimer.pdb   (chains A+B protein, chains C+D estradiol)
        ...
        scripts/dms_Y537_dimer.xml     (FastRelax + InterfaceAnalyzerMover, AB_CD interface)

Output: outputs/dms_Y537_dimer/<tag>_rep<n>.pdb
        outputs/dms_Y537_dimer/dms_Y537_dimer_scores.csv
...
Workflow:
  ...
  2. Each job: clone WT → mutate Y537 on both chain A and chain B → FastRelax
               → InterfaceAnalyzer (AB_CD) → dump PDB.
...
def init_worker(wt_pdb: str, est_params: str, xml_file: str):
    ...
    logger.debug("Worker ready: homodimer holo WT pose loaded, movers configured")


# ── Job runner ─────────────────────────────────────────────────────────────────
def run_dms_job(job: tuple) -> dict | None:
    """Mutate both chains, relax, score protein-ligand interface. Returns score dict or None."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Holo homodimer DMS at Y537 of ERα — symmetric mutation, AB_CD interface"
    )
    ...
    parser.add_argument("--xml",        default=os.path.join(here, "dms_Y537_dimer.xml"),
                        help="RosettaScripts XML (FastRelax + InterfaceAnalyzerMover AB_CD)")
    parser.add_argument("--out_dir",    default=os.path.join(repo, "outputs", "dms_Y537_dimer"),
...
if __name__ == "__main__":
    main()
    logger.info("Homodimer holo DMS complete")
