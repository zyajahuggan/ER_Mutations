...
mkdir -p "${REPO}/logs" "${REPO}/outputs/dms_H524_holo_1GWR"
...
python3 "${REPO}/scripts/dms_H524_1GWR.py" \
    --wt_pdb     "${REPO}/inputs/1GWR_clean.pdb" \
    --est_params "${REPO}/inputs/EST.params" \
    --xml        "${REPO}/scripts/dms_H524_1GWR.xml" \
    --out_dir    "${REPO}/outputs/dms_H524_holo_1GWR" \
    --prefix     1GWR \
    --nstruct    5 \
