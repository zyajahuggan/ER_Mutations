#!/usr/bin/env python3
"""
interface_analyzer_Y537.py

Scores every mutant PDB in four Y537 DMS/1ERE output directories:

  mono_apo    outputs/dms_Y537_apo_1ERE          chain A only, no ligand — no interface,
                                                   just the pose's total Gibbs free energy (dG_complex)
  dimer_apo   outputs/dms_Y537_dimer_apo_1ERE     chains A/B protein dimer, no ligand (A_B interface)
  mono_holo   outputs/dms_Y537_holo_1ERE          chain A protein vs B estradiol    (A_B interface)
  dimer_holo  outputs/dms_Y537_dimer_holo_1ERE    chains AB protein vs CD estradiol (AB_CD interface)

Y537 sits at the dimerization/AF-2 surface, so unlike H524 there is a
meaningful protein-protein interface even without ligand bound (dimer_apo).
...
(Y537Y) replicates. Panels get independent color scales since interface
...
Usage:
    ...
    python3 interface_analyzer_Y537.py --out_csv .../dms_Y537_interface_scores.csv \
                                        --out_fig .../dms_Y537_interface_heatmap.png
    python3 interface_analyzer_Y537.py --plot_only --out_csv <existing csv> --out_fig <png>
"""
import argparse
import csv
import logging
...
CONDITIONS = [
    {
        "name": "mono_apo",
        "pdb_dir": os.path.join(REPO, "outputs", "dms_Y537_apo_1ERE"),
    ...
    {
        "name": "dimer_apo",
        "pdb_dir": os.path.join(REPO, "outputs", "dms_Y537_dimer_apo_1ERE"),
        "interface": "A_B",
        "receptor_chains": "A",
        "ligand_chains": "B",
        "metric": "dG_interface",
    ...
    {
        "name": "mono_holo",
        "pdb_dir": os.path.join(REPO, "outputs", "dms_Y537_holo_1ERE"),
        "interface": "A_B",
        "receptor_chains": "A",
        "ligand_chains": "B",
        "metric": "dG_interface",
    ...
    {
        "name": "dimer_holo",
        "pdb_dir": os.path.join(REPO, "outputs", "dms_Y537_dimer_holo_1ERE"),
        "interface": "AB_CD",
        "receptor_chains": "AB",
        "ligand_chains": "CD",
        "metric": "dG_interface",
...
def run_scoring(out_csv, workers):
    ...
    # baseline dG per condition = mean over that condition's WT (Y537Y) reps
    def _mean(rows, key):
...
def plot_heatmap(csv_path, out_fig):
    ...
    ax_interface.set_xlabel("Mutant residue at Y537")

    fig.suptitle("ERα Y537 DMS — ΔΔG relative to WT (1ERE)", y=1.02)
    fig.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(
        description="ddG scan across the 4 Y537 DMS/1ERE conditions (apo/dimer_apo/holo/dimer_holo)"
    )
    parser.add_argument("--out_csv", default=os.path.join(REPO, "outputs", "dms_Y537_interface_scores.csv"))
    parser.add_argument("--out_fig", default=os.path.join(REPO, "outputs", "dms_Y537_interface_heatmap.png"))
