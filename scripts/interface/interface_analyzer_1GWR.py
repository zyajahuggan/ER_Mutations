#!/usr/bin/env python3
"""
interface_analyzer_1GWR.py

Scores every mutant PDB in the four H524 DMS/1GWR output directories:

  mono_apo    outputs/dms_H524_apo_1GWR          chain A protein, chain C peptide, no ligand
                                                   (A_C peptide interface only -- lone monomer,
                                                    no dimer partner)
  dimer_apo   outputs/dms_H524_dimer_apo_1GWR     chains A/B protein, chains E/F peptide, no ligand
                                                   (A_B protein-protein dimer interface +
                                                    AB_EF peptide interface)
  mono_holo   outputs/dms_H524_holo_1GWR          chain A protein, chain B estradiol, chain C peptide
                                                   (A_B ligand interface + A_C peptide interface)
  dimer_holo  outputs/dms_H524_dimer_holo_1GWR    chains A/B protein, chains C/D estradiol, chains E/F peptide
...
columns follow the same convention interface_analyzer.py already uses for its
own dimer_apo condition: when there's no small-molecule ligand to occupy that
slot (dimer_apo), the A-B protein-protein dimerization interface fills it
...
those extra metrics turned on -- same reason interface_analyzer.py exists
...
condition's own WT (H524H) replicates.

Usage:
    conda activate pyrosetta
    python3 interface_analyzer_1GWR.py --out_csv .../dms_H524_interface_scores_1GWR.csv \
                                        --out_fig .../dms_H524_interface_heatmap_1GWR.png
    python3 interface_analyzer_1GWR.py --plot_only --out_csv <existing csv> --out_fig <png>
"""
import argparse
import csv
import logging
...
CONDITIONS = [
    {
        "name": "mono_apo",
        "pdb_dir": os.path.join(REPO, "outputs", "dms_H524_apo_1GWR"),
        "lig_pair": None,
        "pep_pair": ("A", "C"),
    },
    {
        "name": "dimer_apo",
        "pdb_dir": os.path.join(REPO, "outputs", "dms_H524_dimer_apo_1GWR"),
        "lig_pair": ("A", "B"),      # no small-molecule ligand here -- the A-B protein-protein
                                      ...
                                      # interface_analyzer.py uses for its own dimer_apo condition
        "pep_pair": ("AB", "EF"),
    },
    {
        "name": "mono_holo",
        "pdb_dir": os.path.join(REPO, "outputs", "dms_H524_holo_1GWR"),
        "lig_pair": ("A", "B"),       # real ligand: estradiol
        "pep_pair": ("A", "C"),
    },
    {
        "name": "dimer_holo",
        "pdb_dir": os.path.join(REPO, "outputs", "dms_H524_dimer_holo_1GWR"),
...
def plot_heatmap(csv_path, out_fig):
    ...
    ax_lig.set_xlabel("Mutant residue at H524")

    fig.suptitle("ERα H524 DMS — ΔΔG relative to WT (1GWR + TIF2 peptide)", y=1.02)
    fig.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(
        description="ddG scan across the 4 H524 DMS/1GWR conditions (apo/dimer_apo/holo/dimer_holo), "
                     "dimer/ligand and coactivator-peptide interfaces"
    )
    parser.add_argument("--out_csv", default=os.path.join(REPO, "outputs", "dms_H524_interface_scores_1GWR.csv"))
    parser.add_argument("--out_fig", default=os.path.join(REPO, "outputs", "dms_H524_interface_heatmap_1GWR.png"))
