...
mkdir -p "${REPO}/logs" "${REPO}/outputs/dms/dms_1GWR/dms_Y537_dimer_holo_1GWR"
...
python3 "${REPO}/scripts/dms/dms_Y537_dimer_1GWR.py" \
    --wt_pdb     "${REPO}/inputs/1GWR/1GWR_clean_dimer.pdb" \
    --est_params "${REPO}/inputs/est_ligand/EST.params" \
    --xml        "${REPO}/scripts/dms/dms_Y537_dimer_1GWR.xml" \
    --out_dir    "${REPO}/outputs/dms/dms_1GWR/dms_Y537_dimer_holo_1GWR" \
    --prefix     1GWR \
    --nstruct    5 \
