...
Deep mutation scan at Y537 of the holo ERα monomer + real TIF2 coactivator peptide
(1GWR: chain A protein, chain B estradiol, chain C peptide). Two InterfaceAnalyzerMover
passes score the A_B (protein-ligand) and A_C (protein-peptide) interfaces separately,
so a mutation's effect on ligand binding and coactivator recruitment can be compared.

Input:  --wt_pdb      1GWR holo monomer PDB (chain A protein, chain B estradiol, chain C peptide)
        --est_params   Rosetta params for estradiol
        --xml          RosettaScripts XML (FastRelax + two InterfaceAnalyzerMovers, A_B and A_C)

Output: <out_dir>/<prefix>_<tag>_rep<n>.pdb
        <out_dir>/dms_Y537_scores.csv

Workflow:
  1. Workers init PyRosetta with EST params and load the WT monomer pose once.
  2. Each job: clone WT → mutate Y537 on chain A → FastRelax
               → InterfaceAnalyzer (A_B, ligand) → InterfaceAnalyzer (A_C, peptide) → dump PDB.
...
def init_worker(wt_pdb: str, est_params: str, xml_file: str):
    ...
    logger.debug("Worker ready: monomer holo+peptide WT pose loaded, movers configured")


# ── Job runner ─────────────────────────────────────────────────────────────────
def run_dms_job(job: tuple) -> dict | None:
    """Mutate chain A, relax, score both interfaces. Returns score dict or None."""
    ...
    try:
        ...
        logger.debug(f"InterfaceAnalyzer (ligand A_B): {WT_AA}{DMS_RESNUM}{mut_aa} rep{rep}")
        ...
        logger.debug(f"InterfaceAnalyzer (peptide A_C): {WT_AA}{DMS_RESNUM}{mut_aa} rep{rep}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Holo monomer DMS at Y537 of ERα + TIF2 peptide (1GWR) — A_B and A_C interfaces"
    )
    ...
    parser.add_argument("--xml",        default=os.path.join(here, "dms_Y537_1GWR.xml"),
                        help="RosettaScripts XML (FastRelax + InterfaceAnalyzerMovers A_B, A_C)")
    parser.add_argument("--out_dir",    default=os.path.join(repo, "outputs", "dms_Y537_holo_1GWR"),
    ...
    out_csv = os.path.join(args.out_dir, "dms_Y537_scores.csv")
    if results:
...
if __name__ == "__main__":
    main()
    logger.info("Monomer holo+peptide DMS complete")
