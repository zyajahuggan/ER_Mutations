#!/usr/bin/env python3
"""
dms_H524_apo_1GWR.py

Deep mutation scan at H524 of the apo ERα monomer + real TIF2 coactivator peptide
(1GWR: chain A protein, chain C peptide, no estradiol). InterfaceAnalyzerMover scores
the A_C protein-peptide interface, so coactivator recruitment effects can be measured
even without ligand bound.

Input:  --wt_pdb  1GWR apo monomer PDB (chain A protein, chain C peptide)
        --xml     RosettaScripts XML (FastRelax + InterfaceAnalyzerMover, A_C interface)

Output: <out_dir>/<prefix>_apo_<tag>_rep<n>.pdb
        <out_dir>/dms_H524_apo_scores.csv

Workflow:
  1. Workers init PyRosetta and load the WT monomer pose once.
  2. Each job: clone WT → mutate H524 on chain A → FastRelax
               → InterfaceAnalyzer (A_C, peptide) → dump PDB.
...
def init_worker(wt_pdb: str, xml_file: str):
    ...
    logger.debug("Worker ready: monomer apo+peptide WT pose loaded, movers configured")


# ── Job runner ─────────────────────────────────────────────────────────────────
def run_dms_job(job: tuple) -> dict | None:
    """Mutate chain A, relax, score protein-peptide interface. Returns score dict or None."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Apo monomer DMS at H524 of ERα + TIF2 peptide (1GWR) — A_C interface"
    )
    ...
    parser.add_argument("--xml",     default=os.path.join(here, "dms_H524_apo_1GWR.xml"),
                        help="RosettaScripts XML (FastRelax + InterfaceAnalyzerMover A_C)")
    parser.add_argument("--out_dir", default=os.path.join(repo, "outputs", "dms_H524_apo_1GWR"),
    ...
    out_csv = os.path.join(args.out_dir, "dms_H524_apo_scores.csv")
    if results:
...
if __name__ == "__main__":
    main()
    logger.info("Monomer apo+peptide DMS complete")
