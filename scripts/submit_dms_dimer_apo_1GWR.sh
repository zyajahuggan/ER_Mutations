...
mkdir -p "${REPO}/logs" "${REPO}/outputs/dms_H524_dimer_apo_1GWR"
...
python3 "${REPO}/scripts/dms_H524_dimer_apo_1GWR.py" \
    --wt_pdb  "${REPO}/inputs/1GWR_clean_dimer_apo.pdb" \
    --xml     "${REPO}/scripts/dms_H524_dimer_apo_1GWR.xml" \
    --out_dir "${REPO}/outputs/dms_H524_dimer_apo_1GWR" \
    --prefix  1GWR \
    --nstruct 5 \
