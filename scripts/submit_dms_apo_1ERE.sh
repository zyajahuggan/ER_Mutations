...
mkdir -p "${REPO}/logs" "${REPO}/outputs/dms_H524_apo_1ERE"
...
python3 "${REPO}/scripts/dms_H524_apo.py" \
    --wt_pdb  "${REPO}/inputs/1ERE_clean_apo.pdb" \
    --xml     "${REPO}/scripts/dms_H524_apo.xml" \
    --out_dir "${REPO}/outputs/dms_H524_apo_1ERE" \
    --prefix  1ERE \
    --nstruct 5 \
