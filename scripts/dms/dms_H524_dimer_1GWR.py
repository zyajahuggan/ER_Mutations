#!/usr/bin/env python3
"""
dms_H524_dimer_1GWR.py

Deep mutation scan at H524 of the holo ERα homodimer + real TIF2 coactivator peptides
(1GWR: chains A+B protein, chains C+D estradiol, chains E+F peptide). H524 is mutated
...
Input:  inputs/1GWR/1GWR_clean_dimer.pdb        (chains A+B protein, C+D estradiol, E+F peptide)
        ...
        scripts/dms_H524_dimer_1GWR.xml     (FastRelax + InterfaceAnalyzerMovers AB_CD, AB_EF)

Output: outputs/dms_H524_dimer_holo_1GWR/<tag>_rep<n>.pdb
        outputs/dms_H524_dimer_holo_1GWR/dms_H524_dimer_scores.csv
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Holo homodimer DMS at H524 of ERα + TIF2 peptides (1GWR) — "
    ...
    parser.add_argument("--xml",        default=os.path.join(here, "dms_H524_dimer_1GWR.xml"),
                        help="RosettaScripts XML (FastRelax + InterfaceAnalyzerMovers AB_CD, AB_EF)")
    parser.add_argument("--out_dir",    default=os.path.join(repo, "outputs", "dms_H524_dimer_holo_1GWR"),
    ...
    out_csv = os.path.join(args.out_dir, "dms_H524_dimer_scores.csv")
    if results:
