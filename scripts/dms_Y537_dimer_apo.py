#!/usr/bin/env python3
"""
dms_Y537_dimer_apo.py

Deep mutation scan at Y537 of the apo ERα homodimer (1A52, both chains, no estradiol).
Y537 is mutated symmetrically on both chain A and chain B to model homozygous substitutions.

Input:  inputs/1A52_clean_dimer_apo.pdb  (chains A and B, no ligand)
        scripts/dms_Y537_dimer_apo.xml   (FastRelax, local shell around both Y537 sites)

Output: outputs/dms_Y537_dimer_apo/<tag>_rep<n>.pdb
        outputs/dms_Y537_dimer_apo/dms_Y537_dimer_apo_scores.csv

Workflow:
  1. Workers init PyRosetta and load the WT homodimer pose once.
  2. Each job: clone WT → mutate Y537 on both chain A and chain B → FastRelax → dump PDB.
...
def init_worker(wt_pdb: str, xml_file: str):
    """Load PyRosetta, WT homodimer pose, and FastRelax mover once per worker process."""
    ...
    logger.debug("Worker ready: homodimer apo WT pose loaded, movers configured")


# ── Job runner ─────────────────────────────────────────────────────────────────
def run_dms_job(job: tuple) -> dict | None:
    """Mutate both chains, relax, analyze dimer interface. Returns a score dict or None."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Apo homodimer DMS at Y537 of ERα — symmetric mutation on both chains"
    )
    ...
    parser.add_argument("--xml",     default=os.path.join(here, "dms_Y537_dimer_apo.xml"),
                        help="RosettaScripts XML (FastRelax + InterfaceAnalyzerMover)")
    parser.add_argument("--out_dir", default=os.path.join(repo, "outputs", "dms_Y537_dimer_apo"),
    ...
    out_csv = os.path.join(args.out_dir, "dms_Y537_dimer_apo_scores.csv")
    if results:
...
if __name__ == "__main__":
    main()
    logger.info("Homodimer apo DMS complete")
