#!/usr/bin/env python3
"""
dms_Y537_dimer_apo_1GWR.py

Deep mutation scan at Y537 of the apo ERα homodimer + real TIF2 coactivator peptides
(1GWR: chains A+B protein, chains E+F peptide, no estradiol). Y537 is mutated
...
Input:  inputs/1GWR_clean_dimer_apo.pdb       (chains A+B protein, E+F peptide, no ligand)
        scripts/dms_Y537_dimer_apo_1GWR.xml   (FastRelax + InterfaceAnalyzerMover AB_EF)

Output: outputs/dms_Y537_dimer_apo_1GWR/<tag>_rep<n>.pdb
        outputs/dms_Y537_dimer_apo_1GWR/dms_Y537_dimer_apo_scores.csv
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Apo homodimer DMS at Y537 of ERα + TIF2 peptides (1GWR) — "
    ...
    parser.add_argument("--xml",     default=os.path.join(here, "dms_Y537_dimer_apo_1GWR.xml"),
                        help="RosettaScripts XML (FastRelax + InterfaceAnalyzerMover AB_EF)")
    parser.add_argument("--out_dir", default=os.path.join(repo, "outputs", "dms_Y537_dimer_apo_1GWR"),
