...
Deep mutation scan at H524 of the apo ERα monomer (chain A protein, no ligand).

Input:  --wt_pdb  apo monomer PDB (chain A only)
        --xml     RosettaScripts XML (FastRelax, local shell around H524)
...
Workflow:
  ...
  2. Each job: clone WT → mutate H524 on chain A → FastRelax → dump PDB.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Apo monomer DMS at H524 of ERα"
    )
    parser.add_argument("--wt_pdb",  default=os.path.join(repo, "inputs", "1A52_clean_apo.pdb"),
                        help="Apo monomer PDB (chain A only, no estradiol)")
    parser.add_argument("--xml",     default=os.path.join(here, "dms_H524_apo.xml"),
                        help="RosettaScripts XML (FastRelax)")
    parser.add_argument("--out_dir", default=os.path.join(repo, "outputs", "dms_H524_apo"),
