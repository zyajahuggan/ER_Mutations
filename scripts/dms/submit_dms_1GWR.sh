...
#SBATCH --job-name=ER_DMS_H524_1GWR
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
mkdir -p "${REPO}/logs" "${REPO}/outputs/dms/dms_1GWR/dms_H524_holo_1GWR"
...
# Step 3: DMS
echo "[$(date)] Starting 1GWR holo monomer + peptide DMS at H524 ..."
python3 "${REPO}/scripts/dms/dms_H524_1GWR.py" \
    --wt_pdb     "${REPO}/inputs/1GWR/1GWR_clean.pdb" \
    --est_params "${REPO}/inputs/est_ligand/EST.params" \
    --xml        "${REPO}/scripts/dms/dms_H524_1GWR.xml" \
    --out_dir    "${REPO}/outputs/dms/dms_1GWR/dms_H524_holo_1GWR" \
    --prefix     1GWR \
    --nstruct    5 \
