...
#SBATCH --job-name=ER_DMS_Y537_dimer_1GWR
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
mkdir -p "${REPO}/logs" "${REPO}/outputs/dms_Y537_dimer_holo_1GWR"

# Step 1: Generate holo dimer + coactivator PDB from 1GWR (chains A+B protein, chains
# C+D estradiol, chains E+F TIF2 peptide)
if [[ ! -f "${REPO}/inputs/1GWR_clean_dimer.pdb" ]]; then
    echo "[$(date)] Generating 1GWR holo dimer PDB ..."
    python3 "${REPO}/scripts/clean_1gwr.py" \
        --out_pdb "${REPO}/inputs/1GWR_clean_dimer.pdb" \
        --dimer-holo
else
    echo "[$(date)] Using existing ${REPO}/inputs/1GWR_clean_dimer.pdb"
fi
...
# Step 3: DMS
echo "[$(date)] Starting 1GWR holo homodimer + peptide DMS at Y537 ..."
python3 "${REPO}/scripts/dms_Y537_dimer_1GWR.py" \
    --wt_pdb     "${REPO}/inputs/1GWR_clean_dimer.pdb" \
    --est_params "${REPO}/inputs/EST.params" \
    --xml        "${REPO}/scripts/dms_Y537_dimer_1GWR.xml" \
    --out_dir    "${REPO}/outputs/dms_Y537_dimer_holo_1GWR" \
    --prefix     1GWR \
    --nstruct    5 \
